...
class ProductController extends ChangeNotifier {
  ...
  int? _selectedCategoryId;                         // Currently selected category filter
  ...
  Future<void> fetchHomeData({String? token}) async {
    ...
    try {
      ...
      final results = await Future.wait([
         ...
         _productRepository.getShopProducts(page: 0, sort: 'latest'),
      ]);
  ...
  Future<void> loadMoreShopProducts() async {
    if (_isFetchingMore || !_hasMore || _selectedCategoryId == null) return;
